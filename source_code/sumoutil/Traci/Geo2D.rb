...
module Geo2D
  ...
  class LineSegment < GeoObject
    ...
    ##----------------------------------------
    ## segment point.
    ## 始点から終点に向けてを 1.0 として、その ratio 倍の点。
    def segmentPoint(ratio)
      diff = diffVector() ;
      diff.amplify(ratio);
  ...
  class LineString < GeoObject
    ...
    def to_a()
      array = [] ;
      eachPoint{|point| array.push(point.to_a())}
      array ;
    end
  ...
  class Polygon < GeoObject
    ...
    ##----------------------------------------
    def insidePoint?(point)
      if(@exterior.insidePoint?(point)) then
        eachInterior(){|ring|
          return false if(ring.insidePoint?(point)) ;
        }
        return true ;
      else
        return false ;
    ...
    def to_a()
      array = [] ;
      array.push(@exterior.to_a()) ;
      eachInterior(){|ring|
        array.push(ring.to_a()) ;
      }
  ...
  class Box < GeoObject
    ...
    ##----------------------------------------
    def growBySize(size)  # size must be Vector
      dx = size.x / 2.0 ;
      dy = size.y / 2.0 ;
      @minPos.x -= dx ;
      @maxPos.x += dx ;
    ...
    ##----------------------------------------
    def randomPoint()
      return Point.new(fltRand(@minPos.x, @maxPos.x),
                       fltRand(@minPos.y, @maxPos.y)) ;
    end
